...
from backend.common.models.publication import (
    PublicPublication,
    Publication as AppPublication,
)
...
class PublicationsRepo(OrgRepo[Publication, AppPublication]):
    ...
    async def get_public_publication(
        ...
        query = self._select_by_id(publication_id)
        model = (await session.execute(query)).unique().scalar_one_or_none()
        if model:
            return await self.db_to_public(session, model)
